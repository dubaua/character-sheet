...
character.speed = 30;
character.hitDie = 6;
character.hitLevelUpRolls = [2, 3, 6, 5];
character.hpBonus = 0;
...
character.addSpell({
  ...
  source: "Player's handbook, от класса",
  description: `Вы принимаете другой облик. При накладывании выберите один из представленных ниже вариантов, эффект от которого будет длиться всю длительность заклинания. Пока заклинание активно, вы можете использовать действие магия, чтобы заменить выбранный вами эффект другим.
  <strong>Адаптация к воде.</strong> Вы приспосабливаете своё тело к существованию в воде, отращивая жабры и перепонки между пальцами. Вы можете дышать под водой и получаете скорость плавания, равную вашей скорости.
  ...
  <strong>Естественное оружие.</strong> Не использую и убрал для сокращения места`,
  badges: ['К', 'Х'],
});
...
character.addSpell({
  ...
  source: 'Player\'s Handbook',
  description: `Вы касаетесь объекта на протяжении всего времени накладывания заклинания. Если объект является магическим предметом или иным магическим объектом, то вы узнаете его свойства, как его использовать, требуется ли настройка и сколько у него зарядов, если таковые имеются. Вы узнаете, есть ли на предмете наложенные заклинания и какие именно. Если предмет был создан при помощи заклинания, то вы узнаете название этого заклинания.
Если вместо этого вы касаетесь существа на протяжении времени наложения заклинания, то вы узнаете, какие наложенные заклинания, если таковые есть, действуют на него в данный момент.`,
  badges: ['Х', 'Р'],
});
*/
...
character.addSpell({
  ...
  duration: 'Мгновенная',
  classes: 'волшебник, чародей, жрец (домен света), колдун (исчадие)',
  source: "Player's Handbook",
  description: `Яркая вспышка вырывается от вас к выбранной точке в пределах дистанции и взрывается в огненный шар. Каждое существо в сфере радиусом 20 футов должно совершить спасбросок Ловкости. При провале — 8к6 урона огнём, при успехе — половина.\n\nЛегковоспламеняющиеся объекты в области, которые не носят и не держат, начинают гореть.\n\nНакладывание более высокой ячейкой. Урон увеличивается на 1к6 за каждый уровень ячейки выше 3-го.`,
  badges: ['Х'],
});
character.addSpell({
  ...
  actionType: '10 минут',
  distance: '1 миля',
  components: 'В, С, М (фокусировка стоимостью 100+ зм: украшенный рог или стеклянный глаз)',
  duration: 'Концентрация, до 10 минут',
  classes: 'бард, волшебник, жрец, чародей',
  source: "Player's Handbook",
  description: `Вы создаёте сенсор в пределах дистанции в известном или очевидном месте. Сенсор невидим, неосязаем и неуязвим. Вы можете использовать зрение или слух через него, как если бы находились на его месте. Бонусным действием можно переключаться между этими чувствами.\n\nСущество с Истинным зрением или Видением невидимого видит сенсор как светящийся шар размером с кулак.`,
  badges: ['Х', 'К'],
});

// + ритуал (от черты) + 2 закла 3 уровня + ретрейн возможный + от высшего эльфа закл = туманный шагё
// сигнал тревоги? разговор с животными? поиск фамильяра?
// черта сигнал тревоги
// огненный шарль? 3
// Подсматривание 3
// туманный шаг
